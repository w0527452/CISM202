...
func progressUpdate( steps: Int, goal: Int ) {
  let percent: Double = Double(steps) / Double(goal)
  print("Percent: \(percent)")
  switch percent {
  case let x where x < 0.1:
    print("You're off to a good start.")
    break
  case let x where x > 0.1 && x < 0.5:
    print("You're almost halfway there!")
    break
  case let x where x >= 0.5 && x < 0.9:
    print("You're over halfway there!")
    break
  case let x where x > 0.9 && x < 1.0:
...
progressUpdate(steps: 10, goal: 20)
progressUpdate(steps: 5, goal: 20)
progressUpdate(steps: 19, goal: 20)
progressUpdate(steps: 21, goal: 20)
//:  Your fitness tracking app is going to help runners stay on pace to reach their goals. Write a function called pacing that takes four `Double` parameters called `currentDistance`, `totalDistance`, `currentTime`, and `goalTime`. Your function should calculate whether or not the user is on pace to hit or beat `goalTime`. If yes, print "Keep it up!", otherwise print "You've got to push it just a bit harder!"
func pacing( currentDistance: Double, totalDistance: Double, currentTime: Double, goalTime: Double ){
  let num1 = currentDistance / totalDistance, num2 = currentTime / goalTime
  print("\(num1) \(num2)")
  if num1 == num2 {
    print("keep it up")
  } else if num1 < num2 {
...
pacing(currentDistance: 10, totalDistance: 20, currentTime: 200, goalTime: 400)
pacing(currentDistance: 15, totalDistance: 20, currentTime: 200, goalTime: 400)
pacing(currentDistance: 5, totalDistance: 20, currentTime: 200, goalTime: 400)
