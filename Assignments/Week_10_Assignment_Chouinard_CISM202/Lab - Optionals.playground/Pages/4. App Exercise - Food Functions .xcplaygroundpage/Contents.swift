...
func iAteWhat( meal: String ) -> Meal? {
  if let ate = meals[meal] {
    return ate
  } else {
    return nil
  }
}
print( iAteWhat(meal: "Breakfast")! );
if let dinner = iAteWhat(meal: "Dinner") {
  print( "Dinner: \(dinner)" )
}
/*:
 ...
 */
func getMeals( name: String ) -> [String: Any]? {
  if let meals = UserDefaults.standard.dictionary(forKey: name ) {
    return meals
  } else {
    return nil
  }
}
if let meals = getMeals(name: "test") {
  print( meals )
} else {
